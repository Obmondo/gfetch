...
func (s *Syncer) addBranchUpToDate(result *Result, branch string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	result.BranchesUpToDate = append(result.BranchesUpToDate, branch)
}

func (s *Syncer) addBranchFailed(result *Result, branch string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	result.BranchesFailed = append(result.BranchesFailed, branch)
}

func (s *Syncer) addTagFetched(result *Result, tag string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	result.TagsFetched = append(result.TagsFetched, tag)
}

func (s *Syncer) addTagUpToDate(result *Result, tag string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	result.TagsUpToDate = append(result.TagsUpToDate, tag)
}

func (s *Syncer) addTagFailed(result *Result, tag string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	result.TagsFailed = append(result.TagsFailed, tag)
}

func (s *Syncer) setErr(result *Result, err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if result.Err == nil {
		result.Err = err
...
func (s *Syncer) syncBranches(ctx context.Context, r *git.Repository, repo *config.RepoConfig, auth transport.AuthMethod, opts SyncOptions, branches []*plumbing.Reference, result *Result) {
	if len(branches) == 0 {
		return
	}
...
func logSyncSuccess(ctx context.Context, result Result, duration time.Duration) {
	msg := "sync finished"
...
func (s *Syncer) syncTagsWrapper(ctx context.Context, r *git.Repository, repo *config.RepoConfig, auth transport.AuthMethod, opts SyncOptions, matchedTags []string, result *Result) {
