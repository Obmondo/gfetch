...
func openVoxWorkerCount(repo *config.RepoConfig) int {
	if repo.OpenVoxMaxWorkers == nil || *repo.OpenVoxMaxWorkers <= 0 {
...
func (s *Syncer) syncRepoOpenVox(ctx context.Context, repo *config.RepoConfig, opts SyncOptions) Result {
	...
	ensureProductionAlias(ctx, repo, defaultBranch, remoteBranches)
	if repo.HasProductionAlias() {
...
func extractRemoteRefState(refs []*plumbing.Reference, branchPatterns, tagPatterns []config.Pattern) (string, map[string]struct{}, []*plumbing.Reference, []*plumbing.Reference) {
	...
	matchedTags := make([]*plumbing.Reference, 0)
...
func ensureProductionAlias(ctx context.Context, repo *config.RepoConfig, defaultBranch string, remoteBranches map[string]struct{}) {
	if !repo.HasProductionAlias() {
		return
	}
...
func cleanupOrphanOpenVoxLockFiles(repoName, basePath string, dryRun bool) {
	cleanupOrphanOpenVoxLockFilesInDir(repoName, basePath, basePath, ".gfetch.lock", dryRun)
	cleanupOrphanOpenVoxLockFilesInDir(repoName, basePath, openVoxLocksDir(basePath), ".lock", dryRun)
}

func cleanupOrphanOpenVoxLockFilesInDir(repoName, basePath, scanPath, suffix string, dryRun bool) {
		...
		wg.Go(func() {
			for {
				...
				case ref, ok := <-jobs:
					...
					s.syncOneOpenVoxBranch(ctx, repo, ref, auth, result)
				}
			}
		})
	}
...
func resolveUnresolvedBranchStaleness(ctx context.Context, resolverRepo *git.Repository, cachePath string, repo *config.RepoConfig, opts SyncOptions, auth transport.AuthMethod, unresolved, nonStale []*plumbing.Reference) []*plumbing.Reference {
	...
	for _, ref := range unresolved {
		if isStaleLocal(resolverRepo, ref, opts.StaleAge) {
			telemetry.OpenVoxStaleBranchesSkippedTotal.WithLabelValues(repo.Name).Inc()
			continue
		}
		nonStale = append(nonStale, ref)
...
func syncOpenVoxBranchOnce(ctx context.Context, subCfg *config.RepoConfig, branch string, remoteHash plumbing.Hash, auth transport.AuthMethod, cachePath string) (bool, error) {
	...
	if updated {
		slog.Info("branch updated", "branch", branch)
	}

	return finishOpenVoxBranchSync(ctx, r, branch, updated, filepath.Base(subCfg.LocalPath))
}

func finishOpenVoxBranchSync(ctx context.Context, r *git.Repository, branch string, updated bool, dir string) (bool, error) {
	...
	if stateErr != nil {
		...
		slog.Warn("openvox branch state check failed; forcing checkout", "branch", branch, "dir", dir, "error", stateErr)
		needsCheckout = true
	}

	if dirtyBranch {
		slog.Warn("dirty branch detected, likely manual local changes", "branch", branch, "dir", dir)
	}

	if needsCheckout {
		if err := checkoutRefContext(ctx, r, branch); err != nil {
			return false, err
		}
	}
		...
		wg.Go(func() {
			for {
				...
				case tagRef, ok := <-jobs:
					...
					s.syncOneOpenVoxTag(ctx, repo, tagRef, auth, log, result)
				}
			}
		})
	}
...
func logHashVerifyError(err error, refType, name string) {
	if errors.Is(err, plumbing.ErrReferenceNotFound) {
		slog.Debug(refType+" not found locally, will fetch", refType, name)
		return
	}
	slog.Warn("unable to verify local "+refType+" hash before fetch", refType, name, "error", err)
}

func protectProductionAlias(sanitizedToOriginal map[string]string) {
	sanitizedProduction := SanitizeName(productionAliasName)
	if _, exists := sanitizedToOriginal[sanitizedProduction]; !exists {
		sanitizedToOriginal[sanitizedProduction] = productionAliasName
	}
}
...
func syncOpenVoxTag(ctx context.Context, r *git.Repository, tag string, auth transport.AuthMethod) (bool, error) {
	...
	updated := !errors.Is(err, git.NoErrAlreadyUpToDate)
...
func pruneStaleOpenVoxDirs(ctx context.Context, repo *config.RepoConfig, activeNames map[string]string, staleAge time.Duration, dryRun bool, defaultBranch string, result *Result) {
	...
	for sanitized, original := range activeNames {
		...
		isStale := isOpenVoxDirStale(dirPath, sanitized, cutoff)
		if !isStale {
			continue
		}
...
func isOpenVoxDirStale(dirPath, dirName string, cutoff time.Time) bool {
	r, err := git.PlainOpen(dirPath)
	if err != nil {
		if errors.Is(err, git.ErrRepositoryNotExists) {
